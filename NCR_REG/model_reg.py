...
class EncoderSimilarity(nn.Module):
    ...
    def forward(self, img_emb, cap_emb, cap_lens, return_vec=False):
        sim_all = []
        sim_vecs = []
        ...
        for i in range(n_caption):
            ...
            sim_vecs.append(sim_vec) # shape: (batch, sim_dim)
        ...
        if return_vec:
            sim_vecs = torch.stack(sim_vecs, dim=1).diagonal(dim1=0, dim2=1).permute(1, 0)  # (B, D)
            return sim_all, sim_vecs
        return sim_all
...
class ContrastiveLoss(nn.Module):
    ...
    def forward(
        ...
        if soft_labels is None:
            ...
            margin = self.margin * soft_labels
        elif soft_margin == "exponential":
            s = (torch.pow(10, soft_labels) - 1) / 9
            margin = self.margin * s
        elif soft_margin == "sin":
            s = torch.sin(math.pi * soft_labels - math.pi / 2) / 2 + 1 / 2
            margin = self.margin * s
        ...
        if mode == "predict":
            ...
            return cost_s_mean + cost_im_mean, cost_s_mean.sum() + cost_im_mean.sum()
        elif mode == "train":
            if hard_negative:
                return cost_s_max + cost_im_max, cost_s_max.sum() + cost_im_max.sum()
            else:
                return cost_s_mean + cost_im_mean, cost_s_mean.sum() + cost_im_mean.sum()
        elif mode == "eval":
...
class ELR(nn.Module):
    def __init__(self, beta=0.8, eps=1e-2, exp=2, sim_dim=256, sim_eval_w=None):
        super(ELR, self).__init__()
        self.beta = beta
        self.eps = eps
        self.exp = exp
        self.sim_eval_layer = sim_eval_w
        self.sigmoid = nn.Sigmoid()

        buf_size = 200000
        self.register_buffer("target_scores", torch.zeros(buf_size, sim_dim))
        self.register_buffer("target_vecs", torch.zeros(buf_size, sim_dim))
        self.register_buffer("init", torch.zeros(buf_size).bool())

    def forward(self, sim_vecs, ids):
        targets = self.target_vecs[ids]
        W = self.sim_eval_layer.weight.detach().squeeze(0) # shape: (1, sim_dim)
        b = self.sim_eval_layer.bias.detach() # shape: (1)

        with torch.no_grad():
            # target_ema = self.beta * targets + (1 - self.beta) * sims_diag.detach()
            # self.target_scores[ids] = target_ema
            targets_ema = self.beta * targets + (1 - self.beta) * sim_vecs.detach()
            self.target_vecs[ids] = targets_ema

        # EMA
        # score = (sim_vecs * self.target_vecs[ids]).sum(dim=-1)
        # score = self.sigmoid(score)

        # W
        # score = sim_vecs @ W.T + b # shape: (batch, 1)
        # score = self.sigmoid(score).detach()
        # score = score.squeeze(1) # shape: (batch)

        sim_vecs_norm = F.normalize(sim_vecs, p=2, dim=-1)
        targets_ema_norm = F.normalize(targets_ema, p=2, dim=-1)
        
        score = (sim_vecs_norm * targets_ema_norm).sum(dim=-1)  # shape: (B,)
        reg = torch.log(1 - score.clamp(min=0, max=1-self.eps))
        return reg, reg.sum()
...
class SGRAF(object):
    ...
    def __init__(self, opt):
        ...
        # Loss and Optimizer
        self.criterion = ContrastiveLoss(opt.margin).to(self.device)
        ...
        # ELR hyper-parameters
        self.elr_lambda = opt.elr_lambda
        self.elr_beta = opt.elr_beta
        self.elr_eps = opt.elr_eps
        self.elr_exp = opt.elr_exp
        self.regularization = ELR(
            self.elr_beta,
            self.elr_eps, 
            self.elr_exp, 
            opt.sim_dim,
            self.sim_enc.sim_eval_w,
        ).to(self.device)
    ...
    def forward_sim(self, img_embs, cap_embs, cap_lens, return_vec=False):
        # Forward similarity encoding
        return self.sim_enc(img_embs, cap_embs, cap_lens, return_vec=return_vec)
    ...
    def train(
        ...
        sims, sim_vecs = self.forward_sim(img_embs, cap_embs, cap_lens, return_vec=True)

        # ==================== 변화량 기록 ====================
        # sim_vecs_detached = sim_vecs.detach()
        # step_deltas = []
        # for i, idx in enumerate(ids):
        #     idx = int(idx)
        #     if idx in self.prev_sim_vecs:
        #         delta = torch.norm(sim_vecs_detached[i] - self.prev_sim_vecs[idx], p=2).item()
        #         if i == 1:
        #             print(f"{sim_vecs[i]} - {self.prev_sim_vecs[idx]} =")
        #             print(sim_vecs_detached[i] - self.prev_sim_vecs[idx])
        #         step_deltas.append(delta)
        #     # 업데이트
        #     self.prev_sim_vecs[idx] = sim_vecs_detached[i].clone()

        # if step_deltas:
        #     delta_mean = sum(step_deltas) / len(step_deltas)
        #     self.sim_vec_deltas.append(delta_mean)
        #     print(f"[Eiter {self.Eiters}] sim_vec change (same ID): mean = {delta_mean:.4f}, n = {len(step_deltas)}")

        # measure accuracy and record loss
        self.optimizer.zero_grad()
        contrastive_vals, contrastive_sum = self.criterion(
            sims,
        ...
        elr_vals, elr_sum = self.regularization(
            sim_vecs, ids
        )

        # loss = contrastive_sum + elr_vals.mean()
        loss = contrastive_sum + self.elr_lambda * elr_sum

        # ids = torch.as_tensor(ids, device=self.device)
        # soft_labels = torch.as_tensor(labels, device=self.device)

        # Vectorized ELR update and computation
        # if mode in ["train", "forward"] and self.elr_lambda > 0:
        #     B = sims.size(0)
        #     diag_sim = sims.diag().detach()  # (B,)
        #     ema_old = self.target_scores[ids]  # (B,)
        #     ema_new = self.elr_beta * ema_old + (1 - self.elr_beta) * diag_sim
        #     ema_new = torch.clamp(ema_new, min=1e-6, max=1 - 1e-6)
        #     self.target_scores[ids] = ema_new

        #     sims_pos = sims.diag()
        #     sims_neg_s = (sims.sum(dim=1) - sims.diag()) / B
        #     sims_neg_im = (sims.sum(dim=0) - sims.diag()) / B
        #     margin_gap = sims_pos - (sims_neg_s + sims_neg_im) / 2

        #     gamma = 1
        #     elr_coeff = torch.sigmoid(gamma * margin_gap)

        #     elr_vals = elr_coeff * torch.log(1 - diag_sim * ema_new)  # (B,)
        #     elr_term = elr_vals.mean()
        #     loss += self.elr_lambda * elr_term
    ...
    def eval(self, images, captions, lengths, ids, gt_labels, mode="eval"):
        """
        ...
        with torch.no_grad():
            img_embs, cap_embs, cap_lens = self.forward_emb(images, captions, lengths)
            sims, sim_vecs = self.forward_sim(img_embs, cap_embs, cap_lens, return_vec=True)
            sims_diag = sims.diag().detach()

            contrastive_vals = self.criterion(
                sims,
                hard_negative=True,
                soft_labels=None,
                soft_margin=None,
            ...
            elr_vals, _ = self.regularization(
                sim_vecs, ids
            )
            # ema_old = self.target_scores[ids]
            # elr_vals = torch.log(1 - diag_sim * ema_old)

            if mode == "stat":
                for i, idx in enumerate(ids):
                    stat = {
                        "caption_id": int(idx),
                        "is_clean": bool(gt_labels[idx] == 1),
                        "similarity": float(sims_diag[i].item()),
                        "contrastive": float(contrastive_vals[i].item()),
                        "elr": float(self.elr_lambda * elr_vals[i].item())
                    }
                    per_sample_stats.append(stat)

        return contrastive_vals, elr_vals, per_sample_stats
