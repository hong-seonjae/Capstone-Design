...
from co_train_elr_hn import main
...
def run():
    ...
    parser.add_argument(
        "--data_path", default="/home/capstone_nc/NCR-data/data", help="path to datasets"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--vocab_path",
        default="/home/capstone_nc/NCR-data/vocab",
    ...
    # noise settings
    parser.add_argument("--noise_file", default="f30k_precomp_0.2.npy", help="noise_file") # f30k_precomp_0.2.npy
    ...
    parser.add_argument("--warmup_model_path", default="warmup_model_4.pth.tar", help="warm up models") # warmup_model_8.pth.tar
    parser.add_argument(
    ...
    parser.add_argument("--temperature", default=0.007, type=float, help="Adaptive Hard Negative Temperature")

    # Runing Settings
    parser.add_argument("--gpu", default="1", help="Which gpu to use.")
    parser.add_argument(
        "--seed", default=100, type=int, help="Random seed."
    )
    parser.add_argument(
        "--output_dir", default=os.path.join("output", f"ELR_{current_time}"), help="Output dir."
    )
    
    # save result
    parser.add_argument(
        "--save_validation_result", default="validation.txt", help="set result file name."
    )
    # parser.add_argument(
    #     "--save_loss_result", default="loss.txt", help="set loss file name."
    # )

    # ELR hyper-parameters
    parser.add_argument(
        "--elr_lambda", default=0.2, type=float, help="ELR loss weight"
    )
    parser.add_argument(
        "--elr_beta", default=0.8, type=float, help="ELR EMA decay factor"
    )
    parser.add_argument(
        "--elr_eps", default=5*1e-2, type=float, help="ELR epsilon"
    )
    parser.add_argument(
        "--elr_exp", default=2, type=int, help="ELR exponent for similartiy value"
    )
    '''
    contrastive loss \in [0, 2.4]
    d_sim / d_contrast \in [-2, 2]

    eps = 0.1: s = ema = 0.9, s * ema = 0.81, Loss = -1.34, Grad = -5.92
    eps = 0.05, s = ema = 0.95, s * ema = 0.9025, Loss = -2.10, Grad = -11.58
    eps = 0.01, s = ema = 0.99, s * ema = 0.9801, Loss = -3.839, Grad = -53.1628
    eps가 0.01까지만 내려가도 lambda가 매우 작아져야 한다.

    대충 loss = -0.4, grad = -2 정도가 좋음
    '''
    ...
    if opt.data_name == "coco_precomp":
        ...
        evalrank(
            os.path.join(opt.output_dir, "checkpoints_train", "model_best.pth.tar"),
            split="testall",
            fold5=True,
        )
        print("full validation")
        evalrank(os.path.join(opt.output_dir, "checkpoints_train", "model_best.pth.tar"), split="testall")
    else:
        evalrank(os.path.join(opt.output_dir, "checkpoints_train", "model_best.pth.tar"), split="test")
