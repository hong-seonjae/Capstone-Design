...
class ELR(nn.Module):
    def __init__(self, beta=0.8, eps=1e-1):
        super(ELR, self).__init__()
        self.beta = beta
        self.eps = eps
        self.type = type
        self.init_ema = False
        self.register_buffer("ema", torch.zeros(200000))

    def forward(self, sims, ids):
        sims_diag = sims.diag()

        with torch.no_grad():
            if not self.init_ema:
                self.ema[ids] = sims_diag
            ema_old = self.ema[ids]
            ema_new = self.beta * ema_old + (1 - self.beta) * sims_diag
            ema_new = torch.clamp(ema_new, min=1e-6, max=1-self.eps)
            self.ema[ids] = ema_new
        
        sims_diag = torch.clamp(sims_diag, min=1e-6, max=1-self.eps)
        elr = ema_new * sims_diag * torch.log(1 - ema_new * sims_diag)
        # elr = ema_new * (sims_diag ** 2) * torch.log(1 - ema_new * (sims_diag ** 2))
...
class SGRAF(object):
    ...
    def __init__(self, opt):
        ...
        self.regularization = ELR(self.elr_beta, self.elr_eps).to(self.device)
    ...
    def train(
        ...
        loss = (contrastive_vals + self.elr_lambda * elr_vals).sum()
