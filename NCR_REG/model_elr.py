...
class EncoderSimilarity(nn.Module):
    ...
    def forward(self, img_emb, cap_emb, cap_lens):
        ...
        for i in range(n_caption):
            ...
            # sim_vec = F.normalize(sim_vec, dim=-1)
            # w_norm  = F.normalize(self.sim_eval_w.weight, dim=-1)
            # sim_i = (sim_vec * w_norm).sum(dim=-1, keepdim=True)
            # sim_i = (sim_i + 1) / 2
...
class ELR(nn.Module):
    def __init__(self, beta=0.8, eps=1e-1, exp=1):
        ...
        buf_size = 200000
        self.register_buffer("target_scores", torch.zeros(buf_size, 2))
        self.register_buffer("init", torch.zeros(buf_size).bool())

    def forward(self, sims, ids):
        sims_diag = sims.diag()
        prob = torch.stack((sims_diag, 1 - sims_diag), dim=1)

        t_old = self.target_scores[ids]
        inited = self.init[ids].unsqueeze(1)

        t_new = torch.where(
            inited,
            self.beta * t_old + (1 - self.beta) * prob.detach(),
            prob.detach()
        )
        self.target_scores[ids] = t_new
        self.init[ids] = True

        score = (t_new * prob).sum(dim=1)
        score = score.clamp(min=self.eps, max=1-self.eps)
        reg = torch.log(1 - score)
...
class SGRAF(object):
    ...
    def __init__(self, opt):
        ...
        self.regularization = ELR(self.elr_beta, self.elr_eps, self.elr_exp).to(self.device)
    ...
    def train(
        ...
        reg_vals, reg_sum = self.regularization(
            sims, ids
        )

        # ─── ★ 체크 블록 시작 ★ ──────────────────────────────
        if self.Eiters % 100 == 0:
            # ① contrastive gradient (logit 방향)
            grad_c = torch.autograd.grad(
                        outputs=contrastive_sum,
                        inputs=sims,
                        retain_graph=True)[0]        # (B,B)
            g_c = grad_c.diag().mean()         # 스칼라
            ...
            grad_r = torch.autograd.grad(
                        ...
                        inputs=sims,
                        retain_graph=True)[0]
            g_r = grad_r.diag().mean()

            print(f"[Grad-ratio]  contrastive({g_c:.4f}) / ELR({g_r:.4f}) = {(g_c / g_r + 1e-10):.4f}")

            # lambda 자동 스케일 후 고정
            # if self.elr_lambda < 0:
            #     self.elr_lambda = 0.25 * g_c / (g_r + 1e-12)
            #     print(f"[λ auto-set] elr_lambda = {self.elr_lambda:.4f}")
        # ─── ★ 체크 블록 끝 ★ ────────────────────────────────

        # loss = contrastive_sum + elr_vals.mean()
        loss = 2 * contrastive_sum + self.elr_lambda * reg_sum
    ...
    def eval(self, images, captions, lengths, ids, gt_labels, mode="eval"):
        ...
        with torch.no_grad():
            ...
            sims_prob = torch.sigmoid(sims_diag)
            ...
            if mode == "stat":
                for i, idx in enumerate(ids):
                    stat = {
                        ...
                        "raw_similarity": float(sims_diag[i].item()),
                        "similarity": float(sims_prob[i].item()),
                        "contrastive": float(2 * contrastive_vals[i].item()),
