...
def evalrank(model_path, data_path=None, vocab_path=None, split="dev", fold5=False):
    ...
    summary = f"""
    rsum: {rsum:.1f}
    Average i2t Recall: {ar:.1f}
    Image to text: {r[0]:.1f} {r[1]:.1f} {r[2]:.1f} {r[3]:.1f} {r[4]:.1f}
    Average t2i Recall: {ari:.1f}
    Text to image: {ri[0]:.1f} {ri[1]:.1f} {ri[2]:.1f} {ri[3]:.1f} {ri[4]:.1f}
        """
...
if __name__ == "__main__":
    base_model_dir = "output/2025_04_07_15_16_52"
    data_path = "/home/capstone_nc/NCR-data/data"
    vocab_path = "/home/capstone_nc/NCR-data/vocab"
    eval_dir = os.path.join(base_model_dir, "eval")
    os.makedirs(eval_dir, exist_ok=True)

    ckpt_files = sorted([
        f for f in os.listdir(base_model_dir)
        if f.startswith("checkpoint_") and f.endswith(".pth.tar")
    ], key=lambda x: int(re.findall(r"\d+", x)[0]))

    for ckpt_file in ckpt_files:
        epoch = int(re.findall(r"\d+", ckpt_file)[0])
        ckpt_path = os.path.join(base_model_dir, ckpt_file)
        result_path = os.path.join(eval_dir, f"epoch_{epoch:02d}.txt")
        ...
        try:
            ...
            with open(result_path, "w") as f:
                f.write(f"🔍 Evaluation Result for {ckpt_file}\n")
                f.write("=" * 60 + "\n")
                f.write(summary)
            print(f"✅ [Epoch {epoch}] Saved to {result_path}")
