...
def main(opt):
    ...
    # Negative Learning
    for epoch in range(opt.negative_epochs):
        negative_train(opt, model_A, model_B, noisy_trainloader, epoch)
        print("[{}/{}] Negative Learning model_A".format(epoch + 1, opt.negative_epochs))
        negative_train(opt, model_B, model_A, noisy_trainloader, epoch)
        print("[{}/{}] Negative Learning model_B".format(epoch + 1, opt.negative_epochs))
        ...
        save_checkpoint(
            ...
            False,
            filename="checkpoint_negative_{}.pth.tar".format(epoch),
            prefix=opt.output_dir + "/",
            )
        rsum, r1, r5, r10, r1i, r5i, r10i = validate(opt, val_loader, [model_A, model_B])
        write_file(opt, f"negative_epoch: {epoch} | r1: {r1} | r5: {r5} | r10: {r10} | r1i: {r1i} | r5i: {r5i} | r10i: {r10i}")
    ...
    for epoch in range(opt.num_epochs):
        ...
        pred_A, labeled_idx_A, unlabeled_idx_A = split_prob(prob_A, opt.p_threshold)
        pred_B, labeled_idx_B, unlabeled_idx_B = split_prob(prob_B, opt.p_threshold)

        # save labeled, unlabeled index
...
def negative_train(opt, net, net2, train_loader, epoch=None):
    """
    ...
    progress = ProgressMeter(
        len(train_loader),
        [batch_time, data_time, losses],
        prefix="Negative Co-Teaching Step"
    )
    ...
    for i, (images, captions, lengths, ids) in enumerate(train_loader):
        ...
        # intentionally mismatched: shuffle captions
        idx = torch.randperm(batch_size)
        mismatched_captions = captions[idx]
        mismatched_lengths = [lengths[j] for j in idx]

        # optionally: also shuffle images in reverse
        shuffled_images = images[idx]

        # measure data loading time
        data_time.update(time.time() - end)

        # forward mismatch pairs (img → wrong txt)
        loss_img2txt = net.train(
            images,
            mismatched_captions,
            mismatched_lengths,
            soft_margin=opt.soft_margin,
        ...
        # forward mismatch pairs (txt → wrong img)
        loss_txt2img = net.train(
            shuffled_images,
            captions,
            lengths,
            soft_margin=opt.soft_margin,
        ...
        loss = loss_img2txt + loss_txt2img
        losses.update(loss, batch_size)

        write_file(opt, f"epoch: {epoch} | neg_loss: {loss:.4f}", "neg_loss")
...
def warmup(opt, train_loader, model, mode="warmup"):
    ...
    for i, (images, captions, lengths, _) in enumerate(train_loader):
        ...
        # Update the model
        loss = model.train(images, captions, lengths, mode=mode)
