...
class ContrastiveLoss(nn.Module):
    ...
    def forward(
        ...
        if mode == "predict":
            ...
            alpha = 0.5
            loss_matrix = (scores - alpha).clamp(min=0)

            # mask out diagonal elements (matched pairs)
            eye = torch.eye(scores.size(0), device=scores.device)
            loss_matrix = loss_matrix * (1 - eye)
            
            return loss_matrix.sum()
        
            # scores = scores - self.margin
